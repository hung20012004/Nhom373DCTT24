        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <section className="py-20">
            <div className="mb-12 text-center">
              ...
              <h2 className="text-3xl font-light mt-2">Our Products</h2>
    ...
    <div className="space-y-2">
        <button
            ...
            className={`block w-full text-left px-3 py-2 rounded-lg transition-colors ${
                !filters.category ? 'bg-blue-50 text-blue-700' : 'hover:bg-neutral-50'
            }`}
        >
            All Categories
        </button>
        {categories.map((category) => (
            <button
                key={category.id}
                onClick={() => handleFilterChange('category', category.slug)} // Dùng slug thay vì id
                className={`block w-full text-left px-3 py-2 rounded-lg transition-colors ${
                    ...
                    {products.meta && (
                      <div className="mt-8 flex justify-center">
                        <div className="flex space-x-2">
                          {[...Array(products.meta.last_page)].map((_, index) => (
                            <button
                              key={index}
                              onClick={() => handlePageChange(index + 1)}
                              className={`px-4 py-2 rounded-lg ${
                                products.meta.current_page === index + 1
